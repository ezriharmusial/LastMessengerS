...
<div class="bg-gradient-to-br from-slate-700 to-black w-screen min-h-screen md:p-28 p-6 pt-24">
	<div class="container mx-auto">
		...
		{#if featured && linkcasts?.length}
		<a class="card landscape:flex landscape:flex-row glass-surface landscape:mb-6" href="/linkcasts/{linkcasts[start].slug}#header">
			<header class="bg-black max-h-screen overflow-hidden overflow-y-auto">
				<LinkcastHeader linkcast={linkcasts[start]}/>
			</header>
			<!-- Body -->
			<div class="flex-col">

				<main class="p-4 space-y-4">
					<h5 class="text-primary-500 font-semibold">{linkcasts[start].category || 'News & Announcments'}</h5>

					<h3 class="font-black text-xl">{linkcasts[start].title}</h3>

					<h4 class="font-medium">{#if linkcasts[start].subtitle}{linkcasts[start].subtitle} {/if}<span class="marker text-xs badge badge-filled-surface light:text-black/75 dark:text-white/75">{linkcasts[start].type}</span></h4>

					<article class="unstyled text-2xl ">
						{@html linkcasts[start].preview.html}
					</article>
				...
				<footer class="p-4 flex justify-start items-center space-x-4">
					<Avatar src={linkcasts[start].authorImage || '/images/red-black-purple-gold-with-sun.svg'} />
					<div class="flex-auto flex justify-between items-center">
						<h6 class="font-bold">By {linkcasts[start].author || 'LMS Parrot'}</h6>
						<small>On {new Date(linkcasts[start].date).toLocaleDateString()}</small>
					</div>
				</footer>
			</div>
		</a>
		...
		<div class="landscape:grid landscape:grid-cols-2 landscape:sm:grid-cols-2 landscape:md:grid-cols-2 landscape:xl:grid-cols-3 landscape:grid-flow-dense portrait:flex portrait:flex-col gap-6 mt-6">
			...
			{#each linkcasts.slice((featured ? start + 1 : start), length) as linkcast}
