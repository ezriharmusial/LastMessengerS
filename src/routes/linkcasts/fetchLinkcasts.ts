...
export const fetchLinkcastsMD = Object.entries(import.meta.glob('./**/*.md', { eager: true }))
  .map(([filepath, linkcast]) => {
    ...
    const metadescription = linkcast.metadata.metadescription ? parse(linkcast.metadata.metadescription) : html.querySelector('p', 2)
    let title = linkcast.metadata.title
   	let re = / /gi
