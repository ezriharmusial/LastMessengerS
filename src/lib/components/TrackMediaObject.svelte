...
{#if medium}
<div class="notification { medium.order == $player.track.order ? $player.track.bg_color + ' ' + $player.track.color : ''} blurred has-shadow has-pointer-cursor py-2 px-1">
    <article class="media flex items-center sm:text-md lg:text-2xl" in:fade>
        ...
        <figure class="w-4 h-4 mx-2 lg:w-5 lg:h-5 lg:mx-4">
            <svg id="play-icon px-2 w-3 h-3 lg:w-5 lg:h-5 lg:mx-4" viewBox="0 0 31 37"  xmlns="http://www.w3.org/2000/svg" transition:fade>
