...
<nav class="control h-control-panel px-10 pb-10 rounded-b-xl flex items-center justify-between text-white" in:fade>
	<div class="tracklist opacity-75 hover:opacity-100">
	...
	<div class="shuffle cursor-pointer amplitude-shuffle opacity-75 hover:opacity-100" class:text-yellow-400={$player.shuffle}
	...
	<div class="previous cursor-pointer amplitude-prev opacity-75 hover:opacity-100" class:text-slate-500={!$player.previous} on:click={() => {if ($player.previous) $player.skip('previous')}} id="previous-linkcast"
	...
	<div class="play-pause cursor-pointer amplitude-play-pause w-14 h-14 rounded-full border border-play-pause-light-border shadow-xl flex items-center justify-center bg-slate-300 hover:bg-white dark:bg-play-pause-dark-background dark:border-play-pause-dark-border"
		on:click={ () => $player.playing ? $player.pause() : $player.play() }
	...
	<div class="next cursor-pointer amplitude-next opacity-75 hover:opacity-100" class:text-slate-500={!$player.next}
		on:click={$player.skip('next')} id="next-linkcast"
	...
	<div class="repeat cursor-pointer amplitude-repeat-song opacity-75 hover:opacity-100" class:text-yellow-400={$player.loop != 'no-repeat'}
		on:click={$player.toggleLoop()}
		title="Toggle repeat">
	...
	<div class="like cursor-pointer opacity-75 hover:opacity-100" id="song-saved">
