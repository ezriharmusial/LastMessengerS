...
<nav class="fixed bottom-5 left-0 right-0 control h-control-panel z-5 mx-auto px-5 lg:px-10 rounded-b-xl flex items-center text-center justify-center {$player.track.color} {$player.track.theme == 'dark' ? 'fill-white/75 stroke-white' : 'fill-black/35 stroke-black'} transition-opacity duration-700 opacity-0" class:opacity-100={!$player.playing || $UI.controls.visible}>
	...
	<div class="lyrics cursor-pointer pt-1 mx-1 text-center {$player.track.bg_color} transition-colors duration-1000 opacity-75 hover:opacity-100 h-8 w-8 {$player.track.theme == 'light' ? 'hover:bg-white/20 ' : 'hover:bg-black/20'} rounded-full" id="song-saved"
