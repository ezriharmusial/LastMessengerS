<script>
	import { hasNextTrack, hasPreviousTrack, mediaPlayer, nextTrack, previousTrack, toggleLoop, toggleShuffle } from "$lib/mediaplayer";
...
<nav class="control h-control-panel px-10 pb-10 rounded-b-xl flex items-center justify-between text-white" in:fade>
	<div class="opacity-75 hover:opacity-100">
		<svg width="24" height="24" viewBox="0 0 32 32" fill="currentColor" xmlns="http://www.w3.org/2000/svg" on:click={toggleMenu}>
	...
	<div class="cursor-pointer amplitude-shuffle opacity-75 hover:opacity-100" class:text-yellow-400={$mediaPlayer.shuffle} on:click={toggleShuffle}>
		<svg width="20" height="18" viewBox="0 0 28 26" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
	...
	<div class="cursor-pointer amplitude-prev opacity-75 hover:opacity-100" class:text-slate-500={!hasPreviousTrack} on:click={previousTrack} id="previous-linkcast" title="Pause: {$media.selected.track_artist} – {$media.selected.title}" aria-label="Pause: {$media.selected.track_artist} – {$media.selected.title}" data-post-img-url="{$media.selected.image}">
		<svg width="24" height="24" viewBox="0 0 32 32" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
			...
			<path d="M6 5L6 27" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
		</svg>
	</div>
	<div class="cursor-pointer amplitude-play-pause w-14 h-14 rounded-full border border-play-pause-light-border shadow-xl flex items-center justify-center  bg-slate-300 hover:bg-white dark:bg-play-pause-dark-background dark:border-play-pause-dark-border" on:click={() => $mediaPlayer.player.paused ? $mediaPlayer.player.play() : $mediaPlayer.player.pause()}  title="{$mediaPlayer.player.paused ? 'Play' : 'Pause'}: {$media.selected.track_artist} – {$media.selected.title}" aria-label="{$mediaPlayer.player.paused ? 'Play' : 'Pause'}: {$media.selected.track_artist} – {$media.selected.title}" data-post-img-url="{$media.selected.image}">

		{#if $media?.selected && $mediaPlayer?.player.paused }
		<svg id="play-icon" class="absolute ml-[3px]" width="23" height="30" viewBox="0 0 31 37" fill="currentColor" xmlns="http://www.w3.org/2000/svg" transition:fade>
		...
		<svg id="pause-icon" class="absolute" width="16" height="28" viewBox="0 0 24 36" fill="currentColor" xmlns="http://www.w3.org/2000/svg" transition:fade>
	...
	<div class="cursor-pointer amplitude-next opacity-75 hover:opacity-100" class:text-slate-500={!hasNextTrack} on:click={nextTrack} id="next-linkcast" title="Play Next: {$media.selected.next.track_artist} – {$media.selected.next.title}" aria-label="Play next: {$media.selected.next.track_artist} – {$media.selected.next.title}" data-post-img-url="{$media.selected.next.image}">
		<svg width="24" height="24" viewBox="0 0 32 32" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
			...
			<path d="M26 5L26 27" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
		</svg>
	</div>
	<div class="cursor-pointer amplitude-repeat-song opacity-75 hover:opacity-100" class:text-yellow-400={$mediaPlayer.loop != 'no-repeat'} title="Repeat" on:click={toggleLoop}>
		<svg width="18" height="16" viewBox="0 0 26 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
	...
	<div class="cursor-pointer opacity-75 hover:opacity-100" id="song-saved">
