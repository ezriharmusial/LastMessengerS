...
{#if $player?.track}
<article class="transition-colors duration-1000 fixed w-full h-full top-0 left-0 p-4 portrait:pt-24 {data.bgColor}">
	<header class="transition-colors duration-700 absolute w-full h-full top-0 left-0 {data.color} {$player.track
		.bgColor} {data.bgSize} {$player.track
		.bgBlend} transition-opacity duration-700 {data.bgOpacity} delay-0 {$player.track
		.bgPosition}"
	...
	<main class="absolute top-0 left-0 right-0 bottom-0 h-full {data.alignImage == 'left' ? 'left' : 'right'}">
		<div class="lyrics absolute {$player.track
		.color} portrait:left-0 portrait:right-0 portrait:w-full portrait:px-5 landscape:w-4/5 landscape:sm:w-2/3 landscape:md:w-2/3 landscape:lg:w-2/3 landscape:xl:w-1/2 {data.alignImage ==
			'left'
